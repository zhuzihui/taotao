...
<!-- 	                <a href="javascript:void(0)" class="easyui-linkbutton onePicUpload">图片上传</a>
 -->	            </td>
	        ...
	        <tr>
	            <td>图片2:</td>
	            <td>
	            	选择图片：<input type="file" name="file2" /><br />
...
<script type="text/javascript">
	...
	var contentAddPage  = {
			...
			/* 	$.post("/springsssmxm/content/save",$("#contentAddForm").serialize(), function(data){
					if(data.status == 200){
				...
				$.ajax({
					type:'POST',
					url:'/springsssmxm/content/save',
					processData:false,	//是否对data数据进行处理
					data:new FormData($("#contentAddForm")[0]),
					contentType:false,	//因为form表单中声明了	数据类型
					dataType:'JSON',
					success:function(data){
						if(data.status==200){
							$.messager.alert('提示','新增商品成功!');
							contentAddPage.clearForm();
						}else{
							alert('错误，添加失败..');
						}
					}
